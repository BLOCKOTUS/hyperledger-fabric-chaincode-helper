...
class BlockotusContract extends Contract {
     ...
     */
    static getParams(ctx, rules) {
        const args = ctx.stub.getFunctionAndParameters();
        const params = args.params;

        if (params.length !== rules.length) { throw new Error(`Incorrect number of arguments. Expecting ${rules.length}. Args: ${JSON.stringify(params)}`); }
