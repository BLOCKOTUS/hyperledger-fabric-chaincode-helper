...
class BlockotusContract extends Contract {
    ...
    async didPut(ctx, id, data){
        if (!this.authDIDRequest(ctx, this.name, id)) {
    ...
    async didRequest(ctx, subject, method, data) {
        ...
        if (!this.DIDActionsPerMethod[method]) {
            throw new Error(`${method} Blockotus Method has not been implemented.`);
        }

        const rawDidResponse = await this.DIDActionsPerMethod[method](ctx, parsedSubject, parsedData);
     ...
     * 
     */
     async didAuthDIDRequest(ctx, origin, id) {
